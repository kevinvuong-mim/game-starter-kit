...
# VITE_REPLAY_SECRET: .env.example có sample FRULOOP dev secret; production dùng secret riêng, không commit
VITE_REPLAY_SECRET=b1dec842ef5c5b846eaca346669f2d3ccd9a1811e63d43c249402577207c0820
...
> `coin:spend` wired trong `bindAppEvents()` → `store.spendCoins()`. `shop.purchase()` (coin items) dùng `eventBus.emitAsync('coin:spend')` để đợi handler trước khi `grantItem()`.
...
handler gọi `guest.recoverFromUnauthorized()` → clear `gsk:guest` + reset `notification-state-v1` + clear `game-sync:pending` → `guest.init()` lại.
```
...
Coin purchase flow (`shop.service.ts`):

1. `eventBus.emit('coin:spend', { amount, reason })` → `bindAppEvents()` → `spendCoins()`
2. Nếu không đủ coin → purchase fail
3. `grantItem()` + `shop:purchase` event
...
ScreenManager
ModalScreen             # demo trên HomeScene (đang comment)
ToastManager
...
✅ .env.example có `VITE_REPLAY_SECRET` sample khớp `GAME_CONFIG.FRULOOP` (dev local); production dùng secret riêng trong CI/CD secrets
